...
export const getAllBankListMerchant = (params?: ListParams) =>
  new Promise<ListResponse<{ id: number; name: string; code: string }>>(
    async (resolve, reject) => {
      try {
        const respon = await http.get(`merchant-depo/merchant-depo/bank-list`, {
          params,
        });
        if (respon.data) {
          resolve(respon.data);

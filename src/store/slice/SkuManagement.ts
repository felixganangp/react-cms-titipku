...
interface SkuManagentProps {
  data: SKUs[];
  loading: boolean;
  loadingForm: boolean;
}

const initialState: SkuManagentProps = {
  data: [
    {
      image:
...
const SkuManagementSlice = createSlice({
  name: 'SkuManagement',
  initialState,
  reducers: {
    fetchData(state: SkuManagentProps) {
      state.loading = true;
    },
    fetchDataSuccess(
      state: SkuManagentProps,
      action: PayloadAction<SkuManagentProps>,
    ) {
      state.loading = false;
      state.data = action.payload.data;
    },
    addSkuManagement(state: SkuManagentProps) {
      state.loadingForm = true;
    },
  },
});

export const skuManagementAction = SkuManagementSlice.actions;
export const selectActionManagement = (state: SkuManagentProps) => state.data;
const skuManagementReducer = SkuManagementSlice.reducer;
export default skuManagementReducer;
